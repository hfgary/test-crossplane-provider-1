...
import (
	...
	"github.com/crossplane/provider-myprovidera/apis/v1alpha1"
	myprovidera "github.com/crossplane/provider-myprovidera/internal/controller"
...
func main() {
	var (
		app            = kingpin.New(filepath.Base(os.Args[0]), "MyProviderA support for Crossplane.").DefaultEnvars()
	...
	log := logging.NewLogrLogger(zl.WithName("provider-myprovidera"))
	if *debug {
	...
	mgr, err := ctrl.NewManager(ratelimiter.LimitRESTConfig(cfg, *maxReconcileRate), ctrl.Options{
		...
		LeaderElectionID:           "crossplane-leader-election-provider-myprovidera",
		LeaderElectionResourceLock: resourcelock.LeasesResourceLock,
		LeaseDuration:              func() *time.Duration { d := 60 * time.Second; return &d }(),
		RenewDeadline:              func() *time.Duration { d := 50 * time.Second; return &d }(),
	})
	kingpin.FatalIfError(err, "Cannot create controller manager")
	kingpin.FatalIfError(apis.AddToScheme(mgr.GetScheme()), "Cannot add MyProviderA APIs to scheme")
	...
	if *enableChangeLogs {
		...
		clo := controller.ChangeLogOptions{
			ChangeLogger: managed.NewGRPCChangeLogger(
				...
				managed.WithProviderVersion(fmt.Sprintf("provider-myprovidera:%s", version.Version))),
		}
		o.ChangeLogOptions = &clo
	}

	kingpin.FatalIfError(myprovidera.Setup(mgr, o), "Cannot setup MyProviderA controllers")
